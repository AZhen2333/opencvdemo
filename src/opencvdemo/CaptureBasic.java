...
public class CaptureBasic extends JPanel {
	...
	public static void main(String[] args) {
		try {
			...
			while (frame.isShowing() && n < 500) {
				...
				panel.mImg = panel.mat2BI(detectEyes(capImg));
				panel.repaint();
				// n++;
				// break;
			}
	...
	public static Mat detectEyes(Mat img) throws Exception {

		System.out.println("Running DetectEyes ... ");
		...
		if (rects != null && rects.length == 2) {
			for (Rect rect : rects) {
				Imgproc.rectangle(img, new Point(rect.x, rect.y), new Point(rect.x + rect.width, rect.y + rect.height),
						new Scalar(0, 0, 255), 2);
				System.out.println("检测到两只眼睛");
			}
		} else if (rects != null && rects.length == 1) {
			Rect rect = rects[0];
			Imgproc.rectangle(img, new Point(rect.x, rect.y), new Point(rect.x + rect.width, rect.y + rect.height),
					new Scalar(0, 0, 255), 2);
			System.out.println("检测到已闭眼");

		}else if (rects == null || rects.length == 0) {
			System.out.println("无检测到眼睛");
		}
		return img;
	}
